module fighter
{
    ...
    export class GameContainer extends egret.DisplayObjectContainer
    {
        ...
        private touchHandler(evt:egret.TouchEvent):void{
            ...
            {
                var tx: number = evt.localX - this.myFighter.width / 2.0;
                tx = Math.max(0,tx);
                tx = Math.min(this.stageW-this.myFighter.width,tx);
                this.myFighter.x = tx;
                var ty: number = evt.localY - this.myFighter.height / 2.0;
                ty = Math.max(0,ty);
        ...
        /**创建blood*/
        private createBloods(): void {
            //console.log(this.myFighter.blood); 
            var myBlood: blood.Blood;
            var i: number = 0;
            var myBloodsCount: number = this.myBloods.length;
